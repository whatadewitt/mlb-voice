from flask import Flask, Response, send_file, send_from_directory, request, jsonify
import os
import torch
import time
import subprocess
...
def start_hls():
    ...
    if hls_thread is None or not hls_thread.is_alive():
        ...
        hls_thread.start()
        return jsonify({"status": "HLS stream started"}), 200
    else:
        return jsonify({"status": "HLS stream already running"}), 200
...
def generate():
    ...
    if hls_thread is None or not hls_thread.is_alive():
        return jsonify({"error": "HLS stream not running"}), 500
    
    model.save_audio("./queue/output.wav", output)

    return jsonify({"status": "done", "file": "output.mp3"})
